...
void main() {
	if (fabs(A[0][0]) < fabs(A[0][1]) + fabs(A[0][2]) ||
		...
		printf("%s %10s %11s %12s %15s\n", "Iteration", "unknown", "value", "ea", "max_ea");
		for (int i = 1; i <= 3; i++) {
			if (i == 2)
				printf("%5d %11s %15.5f\n", iter, "x2", x[i - 1]);
			else
				printf("%s %15s%d %15.5f\n", "", "x", i, x[i - 1]);
		}
		printf("\n\n");
		while (maxEa > 5) {
			for (int i = 0; i < MAX_TERM; i++) {
				new_x[0] = rounding(x1(new_x[1], new_x[2], poly), 5);
				new_x[1] = rounding(x2(new_x[0], new_x[2], poly), 5);
				new_x[2] = rounding(x3(new_x[0], new_x[1], poly), 5);
				for (int j = 0; j < MAX_TERM; j++) {
					ea[j] = fabs(((new_x[j] - x[j]) / new_x[j]) * 100);
				}
				...
				for (int j = 1; j <= 3; j++) {
					if (j == 2)
						printf("%5d %11s %15.5f %12.2f%% %12.2f%%\n", ++iter, "x2", new_x[j - 1], ea[j - 1], maxEa);
					else
						printf("%s %15s%d %15.5f %12.2f%%\n", "", "x", j, new_x[j - 1], ea[j - 1]);
				}
				printf("\n\n");
